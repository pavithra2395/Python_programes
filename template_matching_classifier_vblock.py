...
import cv2 
import numpy as np 
import settings_vblock
from utils_vblock import DetectTemplate

def get_template():
    print("Inside get_template Function")
    all_template_ids = [i.get('template_id') for i in settings_vblock.TEMPLATES]
...
def set_flag_for_vblock_empty_or_not(temp_results,locs,w,h,frame):
    flag = 0
    for item in temp_results:
        ...
        for pt in zip(*loc[::-1]): 
            ...
            for bboxes in rectangles:
                ...
                if iou is not None:
                    #print("Vblock is empty")
                    #cv2.putText(frame, "Vblock is empty", (200,40), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,0),  1, cv2.LINE_AA)
                    flag = 1 #set flag for Vblock is empty
                    break        
    return flag
